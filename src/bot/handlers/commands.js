...
🎯 Команды:
/projects - Проекты: открыть, выбрать проект по умолчанию ⭐, задать ключевые слова
/ask - AI-аналитика: задайте вопрос о тратах
/categories - Категории и ключевые слова
/team - Командные и семейные проекты, приглашение участников
/edit - Редактировать последние записи (сумма, описание, категория, проект)
/connect - Подключить свою Google таблицу
/sync - Синхронизация с Google Sheets
/settings - Настройки
/help - Эта справка

📊 Аналитика (просто напишите вопрос или /ask):
• "Сколько потратил на еду в августе?"
• "На что больше всего трачу?"
• "Покажи баланс за месяц"
• "Сравни расходы этого месяца с прошлым"
Также есть кнопка «📊 По проектам» и «📊 Реальность месяца» в меню.

🎯 Проекты по умолчанию и ключевые слова:
• ⭐ Проект по умолчанию: траты без совпадения по ключевым словам идут туда.
  Задаётся в /projects → выбрать проект → «Сделать проектом по умолчанию».
• Ключевые слова проекта определяют, куда автоматически попадёт трата
  (например, "семейный", "общак" → семейный бюджет).
• В командном/семейном проекте общие ключевые слова задаёт создатель,
  но каждый участник может добавить свои личные ключевые слова.

💸 Переводы между проектами:
• Кнопка «💸 Перевод»: выберите откуда → куда → сумму → комментарий.
  Создаётся пара записей (расход + доход), которая не искажает аналитику.

✏️ Редактирование: /edit или «📝 Последние записи» в карточке проекта.
...
async function handleProjects(msg, match) {
  ...
  try {
    ...
    // Create clickable project buttons (⭐ marks the default project)
    const keyboard = projects.map(project => ([{
      text: `📁 ${project.name}${project.id === user.default_project_id ? ' ⭐' : ''}`,
    ...
    let message = '📋 Ваши проекты:\n\n' +
      ...
      '⭐ — проект по умолчанию (туда попадают траты без совпадения по ключевым словам).';
