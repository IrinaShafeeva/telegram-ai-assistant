...
${contextPrompt}ВАЖНО: Если в тексте несколько транзакций (разделены переносами строк, точками с запятой, или перечислены через запятую с отдельными суммами) - верни массив транзакций.
НЕ дели одну покупку только из-за союза "и". Фразы вроде "12 булки и кофе", "мясо и хлеб 20 евро", "продукты и кофе 52" - это одна транзакция, если указана только одна сумма.
...
"12 булки и кофе" → [{"type": "expense", "amount": 12, "currency": null, "description": "Булки и кофе", "category": "Еда", "project": "Личные расходы"}]
"мясо 18 евро" → [{"type": "expense", "amount": 18, "currency": "EUR", "description": "Мясо", "category": "Еда", "project": "Личные расходы"}]
  ...
  async analyzeFinancialData(userQuestion, financialData, userId) {
    try {
      ...
      const advisorProtocol = [
        'РЕЖИМ ФИНКОНСУЛЬТАНТА:',
        '- Отвечай как агент, который ведет бюджет, а не как справочник.',
        '- Сначала дай диагноз в 1 предложении: что происходит с деньгами.',
        '- Затем выдели главный риск или утечку денег, только если это подтверждено данными.',
        '- Дай 1-3 конкретных следующих шага на ближайшие 24 часа или неделю.',
        '- Если данных мало, честно скажи, какой одной записи или суммы не хватает для точного вывода.',
        '- Не морализируй и не пиши общие советы вроде "экономьте больше".',
        '- Если вопрос пользователя про список транзакций, сначала ответь списком, потом дай короткий вывод.'
      ].join('\n');
